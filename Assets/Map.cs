...
public class Map : MonoBehaviour {
    ...
    void DrawRect(Rect zone, Color col) {
        int heightJitter = Random.Range(5, fillJitter);
        int widthJitter = Random.Range(5, fillJitter);
    ...
    void FillZones() {
        ...
        for(int i = 0; i < zones.Count; i++) {
            ...
            if (currentSouv < souvenirLimit) {
                DrawRect(zones.ElementAt(i), souvenir);
                currentSouv++;
                continue;
            }
        
            if (currentWc < wcLimit) {
                DrawRect(zones.ElementAt(i), wc);
                currentWc++;
                continue;
            }

            if (currentFood < foodLimit) {
                DrawRect(zones.ElementAt(i), food);
                currentFood++;
                continue;
            }
            DrawRect(zones.ElementAt(i), attraction);
        }
    }
